const n=`<script type="module">
  import { initializeCitations } from '@/lib/init-citations.js';
  
  // Initialize citations when the page loads
  window.addEventListener('DOMContentLoaded', async () => {
    console.log('[Debug][Template] DOM loaded, initializing citations...');
    try {
      const success = await initializeCitations();
      console.log('[Debug][Template] Citations initialization:', success ? 'successful' : 'failed');
    } catch (error) {
      console.error('[Debug][Template] Failed to initialize citations:', error);
    }
  });
<\/script>
